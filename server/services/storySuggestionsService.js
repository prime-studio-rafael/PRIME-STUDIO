  ...
  async function suggest(input, { signal: externalSignal } = {}) {
    ...
    try {
      ...
      const response = await fetchImpl(`${baseUrl}/chat/completions`, {
        ...
        body: JSON.stringify({ model: modelId, temperature: 0.7, response_format: { type: 'json_object' }, messages: [{ role: 'system', content: systemPrompt() }, { role: 'user', content: JSON.stringify(request) }] }),
      });
      ...
      return validateResponse(body?.choices?.[0]?.message?.content, request.marketingGoal);
    } catch (error) {
...
function validateInput(input = {}) {
  ...
  return { productLabel, sourceCategory, priceText, marketingGoal: input.marketingGoal, tone: input.tone, additionalInstruction };
}

function validateResponse(content, marketingGoal) {
  let parsed;
  ...
  const suggestions = parsed.suggestions.map((item) => validateSuggestion(item, marketingGoal));
  return { suggestions };
}

function validateSuggestion(item, marketingGoal) {
  const suggestion = {
  ...
  for (const field of ['calloutText', 'headline', 'subheadline', 'ctaText']) {
    if (layoutStoryText(suggestion[field], field).blocked) throw new AppError('DEEPSEEK_SUGGESTIONS_INVALID', 'Uma sugestão excede os limites seguros do Story.', { status: 502 });
  }
