...
export function createBatchService({ repository, templateService, config = generationConfig, uuid = randomUUID, now = () => new Date(), estimatePerItemUsd = 0.034 } = {}) {
  ...
  async function create({ name, templateId, files = [] }) {
    ...
    try {
      ...
      const createdAt = iso(); const batch = { id: uuid(), name: normalizedName, templateId: snapshot.publicTemplate.id, templateLabel: snapshot.publicTemplate.label, templateMime: snapshot.image.mimeType, templateDimensions: snapshot.image.dimensions, status: 'ready', totalItems: 0, completedItems: 0, failedItems: 0, cancelledItems: 0, interruptedItems: 0, estimatedCostUsd: Number((items.length * estimatePerItemUsd).toFixed(6)), actualCostUsd: null, createdAt, updatedAt: createdAt, startedAt: null, completedAt: null, pauseRequested: false, cancelRequested: false };
  ...
  async function start(id, { confirmPaid } = {}) { if (!confirmPaid) throw new AppError('CREDIT_CONFIRMATION_REQUIRED', 'Confirme o uso de créditos antes de iniciar o lote.', { status: 400 }); const batch = await repository.update(id, (next) => { if (!['ready', 'paused', 'interrupted'].includes(next.status) || !next.items.some((i) => i.status === 'queued')) throw new AppError('BATCH_NOT_STARTABLE', 'Este lote não possui itens pendentes para iniciar.', { status: 409 }); next.status = 'running'; next.pauseRequested = false; next.cancelRequested = false; next.startedAt ||= iso(); return next; }); queue?.enqueue(id); return publicBatch(batch); }
  ...
  async function executionInput(id, itemId) { const batch = await repository.get(id); const item = batch.items.find((candidate) => candidate.id === itemId); if (!item) throw new AppError('BATCH_ITEM_NOT_FOUND', 'O item do lote não foi encontrado.', { status: 404 }); const garment = await repository.readGarment(id, itemId); const templateSnapshot = await repository.readTemplate(id); return { templateSnapshot, garmentFile: { buffer: garment.buffer, mimetype: garment.mimeType, originalname: garment.originalname, size: garment.size } }; }
