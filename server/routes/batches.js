...
export function createBatchesRouter({ batchService, repository, uploadsDir = path.resolve(process.cwd(), 'storage/.batch-uploads') }) {
  ...
  router.post('/', upload.array('garmentImages', 200), async (req, res, next) => { try { res.status(201).json(await batchService.create({ name: req.body.name, templateId: req.body.templateId, files: req.files || [], additionalInstruction: req.body.additionalInstruction || null })); } catch (error) { next(error); } });
