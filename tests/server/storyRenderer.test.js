...
describe('storyRenderer', () => {
  it.each(['premium', 'luxury', 'minimal', 'offer', 'editorial'])('renders %s deterministically at 1080x1920 WebP and JPEG', async (storyTemplateId) => {
  ...
  it.each(listStoryVisualStyles())('renders $label from its four existing Story fields without a visualStyleId', async (visualStyle) => {
    const output = await renderStory({ sourceBuffer: await image(), logoBuffer: await logo(), story: { ...visualStyle.apply, productLabel: 'Produto Premium', priceText: 'R$ 99,90', headline: 'Destaque da coleção', subheadline: 'Texto de validação', ctaText: 'Ver agora' } });
  ...
  it('keeps the legacy product-highlight Story renderable as Premium', async () => {
    await expect(renderStory({ sourceBuffer: await image(), logoBuffer: await logo(), story: { storyTemplateId: 'product-highlight', productLabel: 'Legado Premium' } })).resolves.toMatchObject({ mimeType: 'image/webp' });
  });
