...
## Melhoria pós-encerramento — Perfil completo de geração por Template (Fase 4: interface)

Estado: **Fase 4 implementada em 21 de julho de 2026, aguardando aprovação do usuário para o encerramento Git**. Fecha a lacuna entre backend (Fases 1-3) e interface: até aqui, nenhum dos 7 campos do perfil de geração era editável, exibido ou enviado pelo frontend, mesmo já sendo retornado em toda resposta de `GET /api/templates`.

- **Formulário de Template** (`TemplateFormModal.jsx`): três novos campos — `prompt` (textarea, 4000 caracteres, com contador), `negativePrompt` (textarea, 2000 caracteres, com contador) e `generationAspectRatio` (`<select>`, só aparece porque hoje já existem 2 valores reais — `1:1`/`4:5`). `provider`/`modelId`/`resolution` **não** ganharam `<select>` nesta fase — hoje só existe uma opção real para cada (`openrouter`/`nano-banana-lite`/`1K`); por instrução explícita, um campo com uma única opção não vira controle de interface. Continuam ajustáveis futuramente do mesmo jeito que `generationAspectRatio`, assim que o catálogo tiver 2+ opções reais.
...
**Validação manual real** (não simulada — servidor de desenvolvimento reiniciado e usado ao vivo neste ambiente): confirmado visualmente que "Tenis 9060" (o Template real que motivou toda esta melhoria) aparecia com "Perfil de geração pendente" e ficava bloqueado tanto em Nova Geração quanto na criação de lote; editado o Template pela interface com um prompt real ("Edite exclusivamente o calçado..."); após salvar, o badge mudou para "Perfil configurado" e o Template passou a aparecer normalmente, selecionável, em ambas as telas — confirmando a correção completa, ponta a ponta, do problema estrutural identificado na auditoria original. Validação também feita em viewport mobile (375×812), sem overflow horizontal. Nenhuma geração real foi executada e nenhuma chamada ao OpenRouter ocorreu durante a validação manual (só edição de Template e navegação).
