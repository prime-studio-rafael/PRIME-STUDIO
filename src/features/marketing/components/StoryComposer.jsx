...
export default function StoryComposer({ week, sources, layouts, story, busy, onSave, onGenerate, onCancel }) {
  ...
  const canRender = required && !warnings.length && (finalUrl || (Boolean(sourceUrl) && imageState === 'ready' && logoState === 'ready'));
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,0.92fr)_minmax(300px,0.72fr)] xl:items-start">
      <form onSubmit={submit} className="order-1 space-y-4">
        ...
        {logoState === 'error' && <div role="alert" className="rounded-xl border border-rose-200 bg-rose-50 p-3 text-xs text-rose-800">A logo aprovada não está disponível. Aprove uma logo em Branding antes de gerar.</div>}
...
export function StoryPreview({ form, sourceUrl, imageState, logoState, onImageLoad, onImageError, onLogoLoad, onLogoError, showSafeArea = false, finalUrl = null }) {
  ...
  return <div><div className="mb-3 flex items-center justify-between"><div><h3 className="text-sm font-semibold text-slate-950">Preview do Story</h3><p className="mt-0.5 text-[11px] text-slate-500">1080 × 1920 · visualização local</p></div><span className="inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-700"><Eye size={13}/> Atualizado agora</span></div><div className="mx-auto w-full max-w-[390px] overflow-hidden rounded-[22px] border-[7px] border-slate-950 bg-slate-950 shadow-xl"><div className="relative w-full overflow-hidden" style={{ aspectRatio: '9 / 16', background: layout.background }}>
    ...
    {!finalUrl && <img
      src="/api/branding/logo?variant=approved"
      alt="Logo aprovada"
      className={`absolute object-contain ${logoState === 'error' ? 'hidden' : ''}`}
      style={styleBox(layout.logo)}
      onLoad={onLogoLoad}
      onError={onLogoError}
    />}
...
function PreviewModal({ form, sourceUrl, imageState, onClose }) { return <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/80 p-4" role="dialog" aria-modal="true" aria-label="Story em tamanho real"><div className="relative flex max-h-full max-w-full flex-col items-center"><button type="button" onClick={onClose} className="mb-3 self-end rounded-full bg-white p-2 text-slate-700" aria-label="Fechar tamanho real"><X size={18}/></button><div className="max-h-[calc(100vh-90px)] w-auto max-w-full"><StoryPreview form={form} sourceUrl={sourceUrl} imageState={imageState}/></div></div></div>; }
...
function normalize(story) { return { ...story, priceText: story.priceText || '', calloutText: story.calloutText || '', headline: story.headline || '', subheadline: story.subheadline || '', ctaText: story.ctaText || '' }; }
