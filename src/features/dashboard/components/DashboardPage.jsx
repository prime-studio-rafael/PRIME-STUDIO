...
export default function DashboardPage({ onNavigate, usdToBrlRate, quoteStatus = 'ready', results = [], resultsStatus = 'idle', resultsError = '', batches = [], batchesStatus = 'idle', batchesError = '', keyConfigured = false, now = new Date() }) {
  ...
  return <div className="space-y-6 pb-6">
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4"><MetricCard label="Imagens geradas" value={String(metrics.generatedCount)} detail="Neste período" icon={FileImage}/><MetricCard label="Tempo médio" value={formatDuration(metrics.averageDurationMs)} detail={metrics.averageDurationMs === null ? 'Sem dados suficientes' : 'Por geração concluída'} icon={Clock3}/><MetricCard label="IA principal" value={metrics.mainModel ? presentModelLabel(metrics.mainModel) : 'Não informado'} valueTitle={metrics.mainModel || undefined} detail={metrics.mainModel ? 'Mais usada no período' : 'Sem modelo registrado no período'} icon={Sparkles}/><MetricCard label="Custo em dólar" value={costUsd === null ? 'Não informado' : formatUsd(costUsd)} detail={costUsd === null ? 'Sem custos registrados no período' : `${metrics.costSampleCount} custo(s) registrado(s)`} accent="text-emerald-700" icon={ArrowUpRight}/><MetricCard label="Custo total em reais" value={brlValue} detail={brlDetail} accent="text-emerald-700" icon={ArrowUpRight}/><MetricCard label="Lotes ativos" value={String(metrics.activeBatches.length)} detail={`${metrics.completedBatches.length} concluído(s) · ${metrics.queueItems} na fila`} icon={Layers3}/><MetricCard label="Taxa de sucesso" value="Não informado" detail="TODO: faltam falhas individuais persistidas" accent="text-slate-600" icon={CheckCircle2}/></section>

    <section className="grid gap-6 xl:grid-cols-[minmax(0,1.55fr)_minmax(320px,0.85fr)]"><SectionCard eyebrow="Produção" title="Produção por dia" description="Histórico disponível após mais produções."><div className="mt-2"><ProductionChart values={[18, 27, 23, 36, 31, 47, 42]}/><div className="mt-2 grid grid-cols-7 text-center text-[10px] font-medium uppercase tracking-wide text-slate-400"><span>Seg</span><span>Ter</span><span>Qua</span><span>Qui</span><span>Sex</span><span>Sáb</span><span>Hoje</span></div></div></SectionCard><SectionCard eyebrow="Atividade" title="Produções recentes" description="Resultados locais mais recentes.">{resultsStatus === 'error' ? <p role="alert" className="mt-6 text-sm text-rose-700">{resultsError || 'Não foi possível carregar os resultados locais.'}</p> : operationalLoading && metrics.recent.length === 0 ? <div className="flex min-h-40 items-center justify-center text-sm text-slate-400"><Loader2 className="animate-spin" size={18}/></div> : metrics.recent.length ? <ol className="mt-1 divide-y divide-slate-100">{metrics.recent.map((item, index) => <li key={`${item.name}-${index}`} className="flex items-center gap-3 py-3 first:pt-1" title={item.absoluteTime}><span className={`h-2 w-2 shrink-0 rounded-full ${item.tone === 'emerald' ? 'bg-emerald-500' : item.tone === 'amber' ? 'bg-amber-400' : item.tone === 'rose' ? 'bg-rose-500' : 'bg-slate-300'}`}/><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-slate-800">{item.name}</p><p className="mt-0.5 text-xs text-slate-400">{item.time}</p></div><span className="rounded-full bg-slate-50 px-2 py-1 text-[10px] font-semibold text-slate-500">{item.label}</span></li>)}</ol> : <p className="mt-6 text-sm text-slate-500">Nenhuma produção local registrada ainda.</p>}<button type="button" onClick={() => onNavigate?.('results')} className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-slate-700 hover:text-slate-950">Ver resultados <ChevronRight size={14}/></button></SectionCard></section>

    <section><div className="mb-2 flex items-center gap-2"><span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-1 text-[10px] font-semibold text-slate-500">Dados demonstrativos</span><span className="text-xs text-slate-400">Histórico local consolidado de Stories e modelos em evolução.</span></div><div className="grid gap-4 lg:grid-cols-3">{distributions.map((item) => <Donut key={item.label} item={item}/>)}</div></section>

    <SectionCard eyebrow="Saúde da operação" title="Tudo sob controle" description="Somente o status local já disponível é exibido; os demais aguardam monitoramento real."><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{health.map(({ label, detail, icon: Icon, status, tone }) => <div key={label} className="rounded-2xl border border-slate-100 bg-slate-50/70 p-4"><div className="flex items-start justify-between"><span className="rounded-xl bg-white p-2 text-slate-600 shadow-sm"><Icon size={16}/></span><span className={`h-2 w-2 rounded-full ${tone === 'sky' ? 'bg-sky-500' : tone === 'amber' ? 'bg-amber-400' : 'bg-slate-300'}`}/></div><p className="mt-4 text-sm font-semibold text-slate-900">{label}</p><p className="mt-1 text-xs text-slate-500">{detail}</p><p className="mt-3 text-[11px] font-semibold text-slate-600">{status}</p></div>)}</div></SectionCard>

    <section className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]"><SectionCard eyebrow="Fila" title="Produção em andamento" description="Lotes locais ativos e seus itens pendentes.">{batchesStatus === 'error' ? <p role="alert" className="mt-6 text-sm text-rose-700">{batchesError || 'Não foi possível carregar os lotes locais.'}</p> : metrics.activeBatches.length ? <div className="space-y-3">{metrics.activeBatches.map((batch) => { const finished = (batch.completedItems || 0) + (batch.failedItems || 0) + (batch.cancelledItems || 0) + (batch.interruptedItems || 0); const progress = batch.totalItems ? Math.round((finished / batch.totalItems) * 100) : 0; return <article key={batch.id} className="rounded-2xl border border-slate-100 p-4 sm:p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-900">{batch.name}</p><p className="mt-1 text-xs text-slate-500">{finished} concluído(s) · {Math.max(0, batch.totalItems - finished)} aguardando</p></div><span className="shrink-0 text-xs font-semibold text-slate-700">{batch.totalItems} imagens</span></div><div className="mt-4 h-1.5 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-slate-900" style={{ width: `${progress}%` }}/></div></article>; })}</div> : <p className="mt-6 text-sm text-slate-500">Nenhum lote local ativo no momento.</p>}<button type="button" onClick={() => onNavigate?.('batches')} className="mt-4 inline-flex items-center gap-2 text-xs font-semibold text-slate-700 hover:text-slate-950">Abrir Produção em Lotes <ArrowRight size={14}/></button></SectionCard><SectionCard eyebrow="Inteligência" title="Insights operacionais" description="Leituras determinísticas, sem chamada de IA."><div className="space-y-3">{insights.map((text) => <Insight key={text} text={text}/>)}</div></SectionCard></section>
