...
export function createBatch({ name, templateId, files }) { const data = new FormData(); data.append('name', name); data.append('templateId', templateId); files.forEach((file) => data.append('garmentImages', file)); return request('/api/batches', { method: 'POST', body: data }); }
